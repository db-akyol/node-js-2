...
router.post("/add", async (req, res) => {
  let body = req.body;
  try {
    if (!body.email) throw CustomError(Enum.HTTP_CODES.BAD_REQUEST, "Validation error", "email must be filled!");
    ...
    if (!body.password) throw CustomError(Enum.HTTP_CODES.BAD_REQUEST, "Validation error", "password field must be filled!");

    if (body.password.length < Enum.PASS_LENGTH) {
      throw new CustomError(Enum.HTTP_CODES.BAD_REQUEST,"Validation Error!","şifre en az 8 haneli olmalı")
    }
...
router.post("/update", async (req, res) => {
  ...
  if (body.password && body.password.length > Enum.PASS_LENGTH) {
    ...
    else{ throw new CustomError(Enum.HTTP_CODES.BAD_REQUEST,"Validation Error!","Password must be at least 8 characters") }
